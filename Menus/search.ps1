...
function Find-Files
{
  ...
  return fd $queryString | fzf --tac --header="Find - In Current Directory" --preview $env:FZF_CUSTOM_PREVIEW
}
...
{
  es -p -r child:^\.git$ | fzf --header="Search - Git Repositories" --preview $env:FZF_CUSTOM_PREVIEW
}
